...
#define WIFI_CONNECT_DELAY_MS                                                  \
  12000 /* 12 s before connecting after provisioning */
...
#define UVC_RETRY_AFTER_OPEN_FAIL_MS 20000  /* 20 s when no camera / USB busy  \
                                             */
#define UVC_SETTLE_AFTER_DISCONNECT_MS                                         \
  300 /* 300 ms after disconnect before stop/close */

/* USB Power */
#define USB_PWR_CTRL_GPIO 1

/* Buttons: 2-user + Boot. Pressure=pressure detection; Multi=capture (USB) or
 * duty cycle (motor) */
#define BTN_PRESSURE_GPIO 6 /* Pressure detection */
#define BTN_MULTI_GPIO                                                         \
  4                     /* Camera: capture. Motor: cycle Off→50%→80%→100%→Off */
#define BTN_BOOT_GPIO 0 /* Long-press: force AP */

#define DEBOUNCE_US 250000

/* MOTOR (L298N) - Motor A: OUT1 & OUT2
 * ENA=GPIO21 (PWM), IN1=GPIO32, IN2=GPIO33. L298N: ENA->GPIO21, IN1->GPIO32,
 * IN2->GPIO33; OUT1/OUT2 to motor. */
#define MOTOR_ENA_GPIO 21
#define MOTOR_IN1_GPIO                                                         \
  32 /* Left header GPIO32 - if 47 didn't drive high, use 32 */
...
/******************** MODES ********************/
typedef enum { MODE_CAMERA = 0, MODE_MOTOR } system_mode_t;

static volatile system_mode_t current_mode =
    MODE_MOTOR; /* USB connected -> CAMERA, else MOTOR */
...
/* Camera: set when physical button pressed; mobile app polls GET
 * /api/capture_request */
static volatile bool capture_requested = false;

/* Capture buffer: one JPEG frame, no re-encode (full quality). 256 KB for 1080p
 * MJPEG. */
...
static void motor_log_task(void *arg) {
  ...
  while (1) {
    if (motor_running != last_running ||
        (motor_running &&
         ledc_get_duty(LEDC_LOW_SPEED_MODE, LED_PWM_CH) != last_duty)) {
...
static void mpu_task(void *arg) {
  ...
  while (1) {
    if (sta_has_ip && mpu_i2c_dev != NULL &&
        i2c_master_transmit_receive(mpu_i2c_dev, &reg, 1, buf, sizeof(buf),
                                    100) == ESP_OK) {
...
static void pwm_init(void) {
  ...
  /* Reset all motor GPIOs to clear any previous configuration (e.g. ADC on
   * GPIO21) */
  ...
  gpio_config_t io_conf = {.pin_bit_mask = (1ULL << MOTOR_IN1_GPIO) |
                                           (1ULL << MOTOR_IN2_GPIO),
  ...
  ESP_LOGI(TAG, "Motor GPIOs (IN1/IN2) config: %s",
  ...
  ledc_channel_config_t ch = {.gpio_num = MOTOR_ENA_GPIO,
   ...
   */
  ESP_LOGI(TAG,
           "GPIO%d (ENA): 50%% PWM for 5s - multimeter DC should show ~1.65V",
           MOTOR_ENA_GPIO);
  ledc_set_duty(LEDC_LOW_SPEED_MODE, LED_PWM_CH,
                4096); /* 50% of 8192 (13-bit) */
...
/* Called only from motor_control_task (task context) so IN1/IN2 and PWM apply
 * correctly */
static void led_set_speed(uint32_t duty) {
  /* Force output mode in case another driver reconfigures these pins (e.g. on
   * ESP32-P4) */
...
/* Task: apply motor commands from queue (task context so GPIO/LEDC work on
 * ESP32-P4) */
static void motor_control_task(void *arg) {
  ...
  while (1) {
    if (xQueueReceive(motor_cmd_queue, &duty, portMAX_DELAY) == pdTRUE) {
      led_set_speed(duty);
      /* Diagnostic: read back IN1/IN2 right after set (if still 0, try
       * different GPIOs e.g. 18,19) */
      if (duty != 0) {
        int i1 = gpio_get_level(MOTOR_IN1_GPIO);
        int i2 = gpio_get_level(MOTOR_IN2_GPIO);
        if (i1 != 1 || i2 != 0)
          ESP_LOGW(TAG,
                   "IN1/IN2 read back %d/%d (expected 1/0) - check wiring to "
                   "GPIO %d,%d",
                   i1, i2, MOTOR_IN1_GPIO, MOTOR_IN2_GPIO);
      }
    }
  }
}
...
static void test_motor_gpios(void) {
  ...
  ESP_LOGI(TAG, "GPIO%d (IN1) read back: %d (expected: 1)", MOTOR_IN1_GPIO,
           level);
  ...
  ESP_LOGI(TAG, "GPIO%d (IN1) read back: %d (expected: 0)", MOTOR_IN1_GPIO,
           level);
  ...
  ESP_LOGI(TAG, "GPIO%d (IN2) read back: %d (expected: 1)", MOTOR_IN2_GPIO,
           level);
  ...
  vTaskDelay(pdMS_TO_TICKS(500));
  level = gpio_get_level(MOTOR_IN2_GPIO);
  ESP_LOGI(TAG, "GPIO%d (IN2) read back: %d (expected: 0)", MOTOR_IN2_GPIO,
           level);
  ...
  ESP_LOGI(TAG, "GPIO%d (ENA) PWM duty read back: %lu (expected: 2048)",
           MOTOR_ENA_GPIO, duty);
  ...
  ESP_LOGI(TAG, "GPIO%d (ENA) PWM duty read back: %lu (expected: 0)",
           MOTOR_ENA_GPIO, duty);
...
static void IRAM_ATTR button_isr(void *arg) {
  ...
  if (level != 0)
    return;
  if (now - last_btn_time[idx] < DEBOUNCE_US)
    return;
  last_btn_time[idx] = now;

  /* Multi button: camera mode = capture; motor mode = cycle
   * Off→50%→80%→100%→Off */
  if (gpio == BTN_MULTI_GPIO) {
    if (current_mode == MODE_CAMERA) {
      ...
      if (motor_cmd_queue) {
        ...
        uint32_t duty =
            (motor_cycle_index == 0) ? 0 : speed_duty[motor_cycle_index - 1];
        xQueueSendFromISR(motor_cmd_queue, &duty, NULL);
      }
      ESP_EARLY_LOGI(TAG, "Motor cycle=%d (duty=%s)", motor_cycle_index,
                     motor_cycle_index == 0   ? "OFF"
                     : motor_cycle_index == 1 ? "50%"
                     : motor_cycle_index == 2 ? "80%"
                                              : "100%");
    }
  }
  ...
  if (gpio == BTN_PRESSURE_GPIO) {
    ESP_EARLY_LOGI(TAG, "Pressure %s",
                   motor_running ? "detected" : "not detected");
  }
}

static void buttons_init(void) {
  gpio_config_t io = {.pin_bit_mask = (1ULL << BTN_PRESSURE_GPIO) |
                                      (1ULL << BTN_MULTI_GPIO) |
                                      (1ULL << BTN_BOOT_GPIO),
  ...
  gpio_isr_handler_add(BTN_PRESSURE_GPIO, button_isr,
                       (void *)BTN_PRESSURE_GPIO);
...
static bool frame_cb(const uvc_host_frame_t *frame, void *arg) {
  if (frame) {
    if (capture_requested && capture_mutex != NULL &&
        frame->data_len <= CAPTURE_BUF_SIZE) {
...
static void stream_event_cb(const uvc_host_stream_event_data_t *event,
                            void *arg) {
...
/* Drain frame queue and return frames to UVC (reduces "Not all frames returned"
 * on disconnect) */
...
static void camera_task(void *arg) {
  /* Try a few common MJPEG modes because some modules (e.g. 1MP GC1009) do not
   * support 1080p. */
  typedef struct {
    uint16_t h_res;
    uint16_t v_res;
  ...
  uvc_host_stream_config_t cfg = {
      ...
      .advanced = {
          ...
          .frame_size = 256 * 1024, /* 256 KB per buffer; 1080p may need PSRAM
                                       for smooth streaming */
          .frame_heap_caps = 0,
#endif
          .number_of_urbs = 6,
          .urb_size = 16 * 1024, /* 16 KB for 1080p payloads */
      }};
  ...
  while (1) {
    ...
    for (size_t i = 0; i < sizeof(try_modes) / sizeof(try_modes[0]); ++i) {
      ...
      if (uvc_host_stream_open(&cfg, pdMS_TO_TICKS(8000), &uvc_stream) ==
          ESP_OK) {
    ...
    if (opened) {
      ...
      /* Let disconnect event settle before stop/close (handles repeated
       * plug/unplug) */
      ...
      uvc_stream = NULL;
      ESP_LOGI(TAG, "USB camera disconnected, waiting %d s before retry",
               (int)(UVC_RETRY_AFTER_DISCONNECT_MS / 1000));
      vTaskDelay(pdMS_TO_TICKS(UVC_RETRY_AFTER_DISCONNECT_MS));
    } else {
      ESP_LOGW(TAG,
               "Could not open camera in tried MJPEG modes; retrying in %d s",
...
static esp_err_t stream_index_handler(httpd_req_t *req) {
  const char *html =
      ...
      ".stream-wrap{background:#000;border-radius:8px;overflow:hidden;max-"
      "width:100%;display:inline-block;}"
      ".stream-wrap img{display:block;max-width:100%;height:auto;}"
      ".note{font-size:0.85rem;color:#999;margin-top:0.5rem;}"
      "</style></head><body>"
      "<h1>Camera stream</h1>"
      "<div class=\"stream-wrap\"><img src=\"/stream\" alt=\"Live stream\" "
      "/></div>"
      "<p class=\"note\">Press device Multi button to capture. Mobile app: "
      "poll GET /api/capture_request for capture trigger.</p>"
      "</body></html>";
...
static esp_err_t stream_handler(httpd_req_t *req) {
  ...
  while (sta_has_ip && current_mode == MODE_CAMERA && uvc_connected) {
    ...
    if (xQueueReceive(frame_queue, &frame, pdMS_TO_TICKS(1500)) == pdPASS) {
      ...
      ret = httpd_resp_send_chunk(req, (const char *)frame->data,
                                  frame->data_len);
      if (ret != ESP_OK)
        break;
...
/* GET /api/capture_request: mobile app polls this; returns {"capture":true}
 * once after button press */
...
/* GET /api/capture_image: returns last captured JPEG (one-shot, full quality,
 * no re-encode) */
static esp_err_t capture_image_handler(httpd_req_t *req) {
  ...
  if (!capture_ready || capture_len == 0) {
    ...
    httpd_resp_send_err(req, HTTPD_404_NOT_FOUND,
                        "No image. Press capture button first.");
    return ESP_FAIL;
  }
...
static void start_stream_server(void) {
  ...
  httpd_uri_t uri_stream = {.uri = "/stream",
                            ...
                            .user_ctx = NULL};
  httpd_uri_t uri_capture_req = {.uri = "/api/capture_request",
                                 .method = HTTP_GET,
                                 .handler = capture_request_handler,
                                 .user_ctx = NULL};
  httpd_uri_t uri_capture_img = {.uri = "/api/capture_image",
                                 .method = HTTP_GET,
...
static void start_mdns(void) {
  ...
  err = mdns_service_add(NULL, "_http", "_tcp", (uint16_t)STREAM_HTTP_PORT, txt,
                         1);
  if (err != ESP_OK) {
    ESP_LOGE(TAG, "mDNS service add failed: %s", esp_err_to_name(err));
  } else {
    ESP_LOGI(TAG, "mDNS: %s.local:%d (discoverable by apps)", MDNS_HOSTNAME,
             (int)STREAM_HTTP_PORT);
  }
}
...
static void save_wifi_to_nvs(void) {
  ...
  ESP_LOGI(
      TAG,
      "WiFi credentials saved to NVS (use Boot long-press to re-provision)");
}
...
static const char provision_html[] =
    ...
    "<script>function "
    "go(){fetch('/wifi',{method:'POST',headers:{'Content-Type':'application/"
    "json'},"
    "body:JSON.stringify({ssid:s.value,pass:p.value})})}</script></body></"
    "html>";
...
static esp_err_t wifi_post_handler(httpd_req_t *req) {
  ...
  buf[n] = '\0';
  int got = sscanf(buf, "{\"ssid\":\"%31[^\"]\",\"pass\":\"%63[^\"]\"}",
  ...
  save_wifi_to_nvs();
  ESP_LOGI(TAG, "Provisioning received: SSID=%s, will switch to STA after %d s",
           saved_ssid, WIFI_CONNECT_DELAY_MS / 1000);
...
static void wifi_task(void *arg) {
  ...
  /* AP with no password so phone/PC can connect for provisioning (avoids
   * "denied access") */
  wifi_config_t ap_cfg = {0};
  strncpy((char *)ap_cfg.ap.ssid, "DentalS3_Provision",
  ...
  if (have_saved && saved_ssid[0] != '\0') {
    ESP_LOGI(
        TAG,
        "Saved WiFi found: %s, connecting (Boot long-press to re-provision)",
        saved_ssid);
    ...
    if (ap_ret != ESP_OK)
      ESP_LOGW(TAG, "AP set_config: %s (co-proc may use default)",
               esp_err_to_name(ap_ret));
    esp_wifi_remote_start();
    ESP_LOGI(TAG, "AP started: SSID=DentalS3_Provision, open. Connect and open "
                  "http://192.168.4.1");
  ...
  while (1) {
    if (provisioning_done) {
      ...
      ESP_LOGI(TAG,
               "Provisioning complete. Waiting %d seconds before connecting to "
               "%s...",
...
void app_main(void) {
  ...
  /* Reduce UVC/USB log spam (frame errors, EP Alloc on disconnect) for clearer
   * serial output */
  ...
  frame_queue = xQueueCreate(16, sizeof(uvc_host_frame_t *));
  capture_mutex = xSemaphoreCreateMutex();

  usb_host_config_t host_cfg = {.skip_phy_setup = false};
  usb_host_install(&host_cfg);

  uvc_host_driver_config_t drv_cfg = {.driver_task_stack_size = 8192,
  ...
  xTaskCreate(motor_log_task, "motor_log", 2048, NULL, 3,
              &motor_log_task_handle);

  ESP_LOGI(TAG, "SYSTEM READY");
  ESP_LOGI(TAG,
           "2-button: Pressure=GPIO%d, Multi=GPIO%d (Camera: capture | Motor: "
           "Off→50%%→80%%→100%%→Off)",
           BTN_PRESSURE_GPIO, BTN_MULTI_GPIO);
  ESP_LOGI(TAG, "Motor GPIOs: ENA=GPIO%d, IN1=GPIO%d, IN2=GPIO%d",
           MOTOR_ENA_GPIO, MOTOR_IN1_GPIO, MOTOR_IN2_GPIO);
}